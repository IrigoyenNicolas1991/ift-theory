...
| **La luz del core / la compuerta única** (¿el nudo lleva materia quiral?) | bitácora madre §33-§35 + actas `PASO-A-bisagra-core-2026-07-21.md` y `PASO-B-bdg-core-2026-07-29.md` | `core_indice_toy.py`, `paso_a_bisagra_core.py`, `paso_b_core_input.py`, `paso_b_bdg_core.py` | **COMPUERTA CERRADA DEL LADO VIVO 2026-07-29**: Paso A — core FM+cíclica alineado, pinning duro (ΔE ×40 el número refutado, QA completo); Paso B — BdG del tubo con el campo real: **C_neto = −1** (una rama Majorana quiral por nudo, cuasi-plana, v/v_F≈0.006, quiralidad esclava del winding — espejo +1 exacto, estable en 4 deformaciones, controles §33 todos PASS); mecanismo probado por vaciado (sin relleno cíclico C_neto=0); NO hay 10ª lápida |
| **El fermión de la red** (¿la molécula carga el U(1)?) | bitácora madre §36 + actas `ASALTO-MOLECULA-dirac-2026-07-29.md` y `ASALTO-MOLECULA-bibliografia-2026-07-29.md` | `asalto_molecula.py`, `asalto_verifica_espB.py` | **PRIMER ASALTO GANADO 2026-07-29: la molécula (vórtice entero = 2 HQV inequivalentes ligados, ΔE=−1.93) lleva un FERMIÓN DE DIRAC QUIRAL CARGADO — C_mol = −2, espejo +2 exacto; LA FASE MANDA** (C_A = C_B = −1 pese a E_A=11.89 ≠ E_B=9.29 y cores distintos; aditividad topológica sobrevive a la reorganización de cores al ligarse {+1,−1}); C_B verificado en 3 capas (perfil radial + fino + caja N=100); novedad barrida en fuentes ANTES (6 huecos, MMN se detiene en k_z=0); 2 erratas propias cazadas el día (K∘T falaz, cruce comido por el grueso); siguen: inflow formal (CH ampara), **Gauss en las uniones (FALTA 2, virgen — nadie sacó fotón de cables)**, tubo→anillo |
